...
kernel void
dwt_hat_transform_row(global float4 *lpass, global float4 *hpass, 
	     int width, int height, const int sc)
{
  const int x = get_global_id(0);
  const int y = get_global_id(1);
  ...
  if (x < sc)
  {
    ...
    lpass[h_idx] = hat_mult * hpass[mad24(y, width, x)] + 
                              hpass[mad24(y, width, (sc - x))] + 
...
kernel void
dwt_hat_transform_col(global float4 *lpass, 
	     int width, int height, const int sc, global float4 *temp_buffer, const float lpass_mult)
{
  const int x = get_global_id(0);
  const int y = get_global_id(1);
  ...
  if (y < sc)
  {
    temp_buffer[mad24(y, width, x)] = (hat_mult * lpass[mad24(y, width, x)] + 
                              lpass[mad24((sc - y), width, x)] + 
                              lpass[mad24((y + sc), width, x)]) * lpass_mult;
  }
  else if (y + sc < size)
  {
    temp_buffer[mad24(y, width, x)] = (hat_mult * lpass[mad24(y, width, x)] + 
  ...
  else if (y < size)
  {
    temp_buffer[mad24(y, width, x)] = (hat_mult * lpass[mad24(y, width, x)] + 
                              lpass[mad24((y - sc), width, x)] + 
                              lpass[mad24((2 * size - 2 - (y + sc)), width, x)]) * lpass_mult;
  }
