...
/*
   ...
   * threads are saved in the table at REGISTRY_INDEX "dt_lua_bg_thread", that table is manipulated with
  save_thread, get_thread, drop_thread. They have a unique integer ID
...
static GSourceFuncs end_job_funcs = {
  end_job_prepare, NULL, /* check */
  end_job_dispatch, NULL,
};
...
static GSourceFuncs stacked_job_funcs = {
  stacked_job_prepare, NULL, /* check */
  stacked_job_dispatch, stacked_job_finalize,
};
...
static GSourceFuncs alien_job_funcs = {
  alien_job_prepare, NULL, /* check */
  alien_job_dispatch, alien_job_finalize,
};
...
static GSourceFuncs string_job_funcs = {
  string_job_prepare, NULL, /* check */
  string_job_dispatch, string_job_finalize,
};
...
void dt_lua_async_call_internal(const char *function, int line, lua_State *L, int nargs, int nresults,
                                dt_lua_finish_callback cb, void *data)
{
#ifdef _DEBUG
...
void dt_lua_async_call_alien_internal(const char *call_function, int line, lua_CFunction pusher, int nresults,
                                      dt_lua_finish_callback cb, void *cb_data,
...
void dt_lua_async_call_string_internal(const char *function, int line, const char *lua_string, int nresults,
                                       dt_lua_finish_callback cb, void *cb_data)
{
#ifdef _DEBUG
