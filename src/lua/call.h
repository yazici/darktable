...
/*
   ...
   * ... triplets of dt_lua_asyc_call_arg_type, type_description, value,
     ...
     * ..._WITH_FREE : the value is a pointer that must be freed after being pushed on the stack
       add a GClosure parameter : the freeing function. will be invoked with the pointer to free plus any extra
   data given at closure creation
 */
...
void dt_lua_async_call_alien_internal(const char *call_function, int line, lua_CFunction function, int nresults,
                                      dt_lua_finish_callback cb, void *data, dt_lua_async_call_arg_type arg_type,
                                      ...);
