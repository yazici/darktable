...
dt_lua_widget_type_t widget_type = {
  .name = "widget", .gui_init = NULL, .gui_cleanup = NULL, .alloc_size = sizeof(dt_lua_widget_t), .parent = NULL
};
...
static void cleanup_widget_sub(lua_State *L, dt_lua_widget_type_t *widget_type, lua_widget widget)
{
  if(widget_type->parent) cleanup_widget_sub(L, widget_type->parent, widget);
...
static void init_widget_sub(lua_State *L, dt_lua_widget_type_t *widget_type)
{
  if(widget_type->parent) init_widget_sub(L, widget_type->parent);
  if(widget_type->gui_init) widget_type->gui_init(L);
}
...
static int widget_gc(lua_State *L)
{
  ...
  // no need to drop, the pointer table is weak and the widget is already being GC, so it's not in the table
  // anymore
...
void dt_lua_widget_register_gtk_callback_type(lua_State *L, luaA_Type type_id, const char *signal_name,
                                              const char *lua_name, GCallback callback)
{
