...
static void on_child_added(GtkContainer *container, GtkWidget *child, lua_container user_data)
{
  dt_lua_async_call_alien(dt_lua_widget_trigger_callback, 0, NULL, NULL, LUA_ASYNC_TYPENAME, "lua_widget",
                          user_data, LUA_ASYNC_TYPENAME, "const char*", "add", LUA_ASYNC_TYPENAME, "lua_widget",
                          child, LUA_ASYNC_DONE);
}

static void on_child_removed(GtkContainer *container, GtkWidget *child, lua_container user_data)
{
  dt_lua_async_call_alien(dt_lua_widget_trigger_callback, 0, NULL, NULL, LUA_ASYNC_TYPENAME, "lua_widget",
                          user_data, LUA_ASYNC_TYPENAME, "const char*", "remove", LUA_ASYNC_TYPENAME, "lua_widget",
                          child, LUA_ASYNC_DONE);
}
...
{
  lua_widget widget;
  luaA_to(L, lua_widget, &widget, 2), lua_getuservalue(L, 1);
  luaA_push(L, lua_widget, &widget);
  lua_pushnil(L);
  lua_settable(L, -3);
  return 0;
...
static int container_numindex(lua_State *L)
{
  ...
  if(lua_gettop(L) > 2)
  {
    ...
    if(!lua_isnil(L, 3) && index == length)
    {
      ...
      // the following lines add the widget to the container's user_value to guarentee it's referenced on the lua
      // side
    ...
    {
      luaL_error(L, "Incorrect index or value when setting the child of a container : you can only append, not "
                    "change a child in the middle");
    }
    g_list_free(children);
    return 0;
  }
