		...
		const float denoise)
{
  ...
  val -=  c * dt_fast_expf(-c*c/(2.0*denoise*denoise/3.0f));
  return val;
}
...
#pragma omp parallel for default(none) schedule(dynamic)
  ...
  for(uint32_t j=padding;j<h-padding;j++)
  {
    ...
    for(;out2<beg;out2++,in2++)
      *out2 = curve_scalar(*in2, g, sigma, shadows, highlights, clarity, denoise);
...
#if defined(__SSE2__)
    if(use_sse2)
      apply_curve_sse2(buf[k][0], padded[0], w, h, max_supp, gamma[k], sigma, shadows, highlights, clarity, denoise);
    else // brackets in next line needed for silly gcc warning:
#endif
    {apply_curve(buf[k][0], padded[0], w, h, max_supp, gamma[k], sigma, shadows, highlights, clarity, denoise);}
