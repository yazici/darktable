...
static void rlucy_check_nan(rlucy_image_t *im, const char *str)
{
  ...
  for(int i = 0; i < im->w * im->h * im->h; i++)
    if(isnan(im->im[i])) i_nan++;
...
const char *rlucy_time_desc[MAX_TIME]
    = { "dummy         ", "fft lock    ",   "fft execute ",   "fft copy    ",   "fft mult    ",
        "fft plan    ",   "convolve      ", "sum           ", "subtract      ", "mulsub        ",
        "add           ", "pad image     ", "unpad image   ", "trim mask     ", "divTV         ",
        "rotate        ", "gradient      ", "divide        ", "process       ", "build kernel  " };
...
static void rlucy_resize_1c(rlucy_image_t *image_src, rlucy_image_t *image_dest, const float scale,
                            const int new_w, const int new_h, rlucy_image_t *im_tmp_in, rlucy_image_t *im_tmp_out)
{
  float *out = NULL;
  float *in = NULL;
...
static void rlucy_resize(rlucy_image_t *image_src, rlucy_image_t *image_dest, const float scale, const int new_w,
                         const int new_h, rlucy_image_t *im_tmp_in, rlucy_image_t *im_tmp_out)
{
  if(image_src->c == 1)
  {
...
static void rlucy_img_sum(const rlucy_image_t *const image, float *sum, double *time)
{
  ...
  for(int c = 0; c < image->c; c++) sum[c] = 0.f;

  for(int i = 0; i < size; i += image->c)
  {
    for(int c = 0; c < image->c; c++) sum[c] += image->im[i + c];
  }
...
#pragma omp parallel for default(none) schedule(static)
  ...
  for(int i = 0; i < size; i += image->c)
  {
    for(int c = 0; c < image->c; c++) image->im[i + c] = fabs(image->im[i + c]);
  }
...
static void rlucy_img_subtract(rlucy_image_t *img_in1, const rlucy_image_t *const img_in2, double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
...
static void rlucy_img_add3(rlucy_image_t *img_in1, const rlucy_image_t *const img_in2, float *add, double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(img_in1_im, add, img_in1) schedule(static)
#endif
  for(int i = 0; i < size; i += img_in1->c)
  {
    for(int c = 0; c < img_in1->c; c++) img_in1_im[i + c] += img_in2_im[i + c] + add[c];
  }

  RLUCY_TIME_END(TIME_SUBTRACT)
}

static void rlucy_img_mulsub(rlucy_image_t *img_in1, const rlucy_image_t *const img_in2, const float *const mult,
                             double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
...
static void rlucy_img_divadd(rlucy_image_t *img_in1, const rlucy_image_t *const img_in2, const float *const add,
                             double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
...
static void pad_image(const rlucy_image_t *const img_src, const int pad_h, const int pad_v,
                      rlucy_image_t *img_dest, rlucy_image_t *img_tmp1, double *time)
{
  ...
  {
    pad_image(img_src, pad_h, pad_v, img_tmp1, img_tmp1, time);
  ...
  else
  {
    ...
    convolve_pad_image(img_src->im, img_src->w, img_src->h, img_src->c, pad_h, pad_v, img_dest->im, img_dest->w,
                       img_dest->h, time);
  }
...
static void unpad_image(rlucy_image_t *img_dest, const int pad_h, const int pad_v, rlucy_image_t *img_src,
                        rlucy_image_t *img_tmp1, double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
  ...
  if(img_src->im == img_dest->im)
  {
    unpad_image(img_tmp1, pad_h, pad_v, img_src, img_tmp1, time);
    rlucy_alloc_image(img_dest, img_tmp1->w, img_tmp1->h, img_tmp1->c);
    rlucy_copy_image(img_dest, img_tmp1);
  }
  else
...
static void rlucy_convolve(const rlucy_image_t *const image, const rlucy_image_t *const kernel, const int mode,
                           rlucy_image_t *img_dest, convolve_data_t *fft_conv_data, double *time)
{
  ...
  convolve_get_dest_size(image->w, image->h, kernel->w, kernel->h, &w, &h, mode);

  rlucy_alloc_image(img_dest, w, h, image->c);

  memset(img_dest->im, 0, img_dest->w * img_dest->h * img_dest->c * sizeof(float));

  convolve(fft_conv_data, image->im, image->w, image->h, image->c, kernel->im, kernel->w, kernel->h, img_dest->im,
           img_dest->w, img_dest->h, mode, &(time[TIME_CONV_FFT_LOCK]));
...
static void rlucy_img_rotate90(const rlucy_image_t *const img_in, rlucy_image_t *img_dest, const int times,
                               double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
...
static void rlucy_trim_mask(const rlucy_image_t *const img_src, rlucy_image_t *img_dest, const int im_x,
                            const int im_y, const int im_w, const int im_h, rlucy_image_t *im_tmp1, double *time)
{
  RLUCY_TIME_DECL
  RLUCY_TIME_BEGIN
  ...
  else
  {
    ...
    if(img_dest->w > img_src->w || img_dest->h > img_src->h) printf("rlucy_trim_mask: roi_dest > roi_src\n");
...
static void rlucy_img_norm(rlucy_image_t *image, float *norm, double *time)
{
  ...
  for(int c = 0; c < image->c; c++) norm[c] = sqrtf(norm[c]);
}
...
static void rlucy_img_norm2(rlucy_image_t *image1, rlucy_image_t *image2, float *norm, double *time)
{
  ...
  for(int c = 0; c < image1->c; c++) norm[c] = sqrtf(norm1[c] + norm2[c]);
}
...
static void best_param(rlucy_image_t *image, float *lambd, int p, float *epsilon, double *time)
{
  ...
  */
  ...
  for(int c = 0; c < image->c; c++) grad_std[c] = norm[c] / (float)(image->w * image->h);

  /* grad_mean = np.linalg.norm(grad) / image.size */
  rlucy_img_norm2(&(grad[0]), &(grad[1]), grad_mean, time);
  for(int c = 0; c < image->c; c++) grad_mean[c] = grad_mean[c] / (float)(image->w * image->h);

  // # lambd = noise_reduction_factor * np.sum(np.sqrt(divTV(image, p=1)))**2 / (-np.log(np.std(image)**2) * * 2 *
  // np.pi)

  /* omega = 2 * lambd * grad_std / p */
  for(int c = 0; c < image->c; c++) omega[c] = 2.f * lambd[c] * grad_std[c] / (float)p;
...
static void divTV(rlucy_image_t *u, float *epsilon, int p, rlucy_image_t *divtv, double *time)
{
  ...
  // # For Darktable implementation, don't bother to implement the p parameter, just use the p = 1 case to optimize
  // the computations
  if(p == 1)
  {
...
*/
static void gradTVEM(rlucy_image_t *u, rlucy_image_t *ut, float *epsilon /* = 1e-3 */, float *tau /* = 1e-1 */,
                     float p /* = 0.5 */, rlucy_image_t *img_dest, double *time)
{
...
*/
static void _convolve_image(const rlucy_image_t *const u, const rlucy_image_t *const image,
                            const rlucy_image_t *const psf, rlucy_image_t *dest, rlucy_image_t *tmp1,
                            rlucy_image_t *tmp2, convolve_data_t *fft_conv_data, double *time)
{
...
*/
static void _convolve_kernel(const rlucy_image_t *const u, const rlucy_image_t *const image,
                             const rlucy_image_t *const psf, rlucy_image_t *dest, rlucy_image_t *tmp1,
                             rlucy_image_t *tmp2, convolve_data_t *fft_conv_data, double *time)
{
  /*    error = np.ascontiguousarray(convolve(u, psf, "valid"), np.float32)
...
*/
static void richardson_lucy_PAM(rlucy_image_t *image, rlucy_image_t *u, rlucy_image_t *psf, float *lambd,
                                int iterations, float *epsilon /* = 1e-3 */, int *mask /* = None */,
                                int blind /* = True */, convolve_data_t *fft_conv_data, double *time)
{
  ...
  */
  ...
  for(int it = 0; it < iterations; it++)
  {
    ...
    */
    ...
    for(int c = 0; c < image->c; c++) lambd[c] = MIN(lambd[c], 50000.f);
    ...
    for(int itt = 0; itt < 5; itt++)
    {
      ...
      for(int c = 0; c < image->c; c++) lambd[c] *= 1.001;
    }
  }
...
static void richardson_lucy_MM(rlucy_image_t *image, rlucy_image_t *u, rlucy_image_t *psf, float *lambd,
                               int iterations, float *epsilon /* = 5e-3 */, int *mask /* = None */,
                               int blind /* = True */, convolve_data_t *fft_conv_data, double *time)
{
  /*
      """Richardson-Lucy Blind and non Blind Deconvolution with Total Variation Regularization by the
...
static void build_pyramid(const int psf_size, const float lambd, int method, int **kernels_size,
                          float **images_scaling, float **lambdas, int *pyramid_len)
{
  /*
      To speed-up the deconvolution, the PSF is estimated successively on smaller images of increasing sizes. This
     function
  ...
  */
  ...
  if(arr_kernels == NULL || arr_images_scaling == NULL || arr_lambdas == NULL) goto cleanup;
...
static void process_pyramid(rlucy_image_t *pic, rlucy_image_t *u, rlucy_image_t *psf, float lambd, int method,
                            float epsilon /* = 1e-3 */, int quality /* = 1 */, convolve_data_t *fft_conv_data,
                            double *time)
{
  /*
      To speed-up the deconvolution, the PSF is estimated successively on smaller images of increasing sizes. This
  ...
  */
  ...
  //  rlucy_resize(u, u, arr_images_scaling[pyramid_len-1], u->w * arr_images_scaling[pyramid_len-1], u->h *
  //  arr_images_scaling[pyramid_len-1], &img_tmp1, &img_tmp2);
  ...
  for(int i = pyramid_len - 1; i >= 0; i--)
  {
    ...
    if(arr_images_scaling[i] != 1.f)
    {
      ...
      rlucy_resize(pic, &im, arr_images_scaling[i], pic->w * arr_images_scaling[i], pic->h * arr_images_scaling[i],
                   &img_tmp1, &img_tmp2);
    }
    else
    {
...
/*static float gaussian_function(int x, int mu, float sigma) {
    return expf(- (powf(x - mu, 2)) / (2.f * powf(sigma, 2) )) / (sigma * sqrtf(2.f * M_PI));
}


static float gaussian_vector(int size, float sigma) {

    int mu = ceilf(size / 2) + 1;
    float vect[size] = {0};

    for (int i = 0; i < size; i++) {
        vect[i] = gaussian_function(i, mu, sigma);
    }
...
static void rlucy_gaussian_kernel(rlucy_image_t *kern, const float beta, double *time)
{
  ...
  if(kern->w != kern->h) printf("rlucy_gaussian_kernel: invalid kernel size\n");
...
void rlucy_deblur_module(float *pic_in, int blur_type, int blur_width, float noise_reduction_factor, float quality,
                         float ringing_factor, int *mask, int method, float *pic_out, int width, int height,
                         int ch)
{
  ...
  float noise_reduction_factor_a[4]
      = { noise_reduction_factor, noise_reduction_factor, noise_reduction_factor, noise_reduction_factor };
  ...
  // Make the picture dimensions odd to avoid ringing on the border of even pictures. We just replicate the last
  // row/column
  int odd_vert = 0;
  int odd_hor = 0;
  ...
  if(blur_type == rlucy_blur_type_auto)
  {
    ...
    process_pyramid(&pic, &u, &psf, noise_reduction_factor, method, ringing_factor, quality, &fft_conv_data, time);
  }
