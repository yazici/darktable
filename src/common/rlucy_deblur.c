...
*/
static void gradTVEM(rlucy_image_t *u, rlucy_image_t *ut, float *epsilon /* = 1e-3 */, float *tau /* = 1e-1 */, float p /* = 0.5 */, rlucy_image_t *img_dest,
    double *time)
{
...
*/
static void _convolve_image(const rlucy_image_t *const u, const rlucy_image_t *const image, const rlucy_image_t *const psf, rlucy_image_t *dest,
...
*/
static void _convolve_kernel(const rlucy_image_t *const u, const rlucy_image_t *const image, const rlucy_image_t *const psf, rlucy_image_t *dest,
    rlucy_image_t *tmp1, rlucy_image_t *tmp2,
    convolve_data_t *fft_conv_data, double *time)
{
  /*    error = np.ascontiguousarray(convolve(u, psf, "valid"), np.float32)
...
*/
static void process_pyramid(rlucy_image_t *pic, rlucy_image_t *u, rlucy_image_t *psf, float lambd, int method, float epsilon /* = 1e-3 */, int quality /* = 1 */,
    convolve_data_t *fft_conv_data, double *time)
{
/*
    To speed-up the deconvolution, the PSF is estimated successively on smaller images of increasing sizes. This function
...
//  rlucy_resize(u, u, arr_images_scaling[pyramid_len-1], u->w * arr_images_scaling[pyramid_len-1], u->h * arr_images_scaling[pyramid_len-1], &img_tmp1, &img_tmp2);
  ...
  // # TODO : pad the picture to make its dimensions odd
      ...
      rlucy_resize(pic, &im, arr_images_scaling[i], pic->w * arr_images_scaling[i], pic->h * arr_images_scaling[i], &img_tmp1, &img_tmp2);
    }
    else
    {
      /* im = pic.copy() */
...
static void build_kernel(rlucy_image_t *psf, int blur_type, int blur_width, int blur_strength, int ch, double *time)
{
  ...
  else if (blur_type == rlucy_blur_type_kaiser)
  {
 ...
 */
    ...
    // FIXME: do we need to return u here?
