...
static void _rlucy_check_nan(const float* im, const int size, const char *str)
{
	int i_nan = 0;
...
#define rlucy_check_nan(im, size, str) _rlucy_check_nan(im, size, str)
 
#else
 
#define rlucy_check_nan(im, size, str) {}
...
static int _len_guards(const int M)
{
   // """Handle small or incorrect window lengths"""
    if (M < 0)
        printf("Window length M must be a non-negative integer");
...
/*	if _len_guards(M):
	...
	if (_len_guards(win_size))
	{
		for (int i = 0; i < win_size; i++) win[i] = 1.f;
	}
	else
	{
...
void rlucy_gaussian_kernel(float *kern, const dt_iop_roi_t *const roi_kern, const int ch, const float beta)
{
	double time = 0;
...
/*def uniform_kernel(size):
    ...
    kern /= np.sum(kern)
    return kern*/
void rlucy_uniform_kernel(float *kern, const dt_iop_roi_t *const roi_kern, const int ch)
{
	const int radius = roi_kern->width;
	...
	for (int i = 0; i < roi_kern->width * roi_kern->height * ch; i++)
	{
		kern[i] = radius2;
	}
}
		...
		float **img_dest, dt_iop_roi_t * roi_dest)
{
	...
	{
		img_padded = *img_dest;
		if (roi_dest->width != roi_src->width + pad_h * 2 || roi_dest->height != roi_src->height + pad_v * 2)
			printf("rlucy_pad_image: invalid dest size\n");
	}
	
//	memset(img_padded, 0, roi_dest->width * roi_dest->height * ch * sizeof(float));
...
//		float *dest_r = img_padded + (y + pad_v) * roi_dest->width * ch + (roi_src->width+pad_h-1) * ch;
		float *dest_r = img_padded + (y + pad_v) * roi_dest->width * ch + (roi_src->width+pad_h) * ch;
...
cleanup:
	...
	if (*img_dest == NULL) *img_dest = img_padded;
...
static void rlucy_trim_mask(const float *const img_src, const dt_iop_roi_t *const roi_src, const int ch,
		...
		float **img_dest, dt_iop_roi_t * roi_dest)
{
	...
	if (*img_dest == NULL)
	{
		*roi_dest = *roi_src;
		...
		roi_dest->x = mask[0];
		roi_dest->y = mask[1];
		roi_dest->width = mask[2];
		roi_dest->height = mask[3];
		
		img_trim = dt_alloc_align(64, roi_dest->width * roi_dest->height * ch * sizeof(float));
		if (img_trim == NULL) goto cleanup;
	}
	else
	{
		img_trim = *img_dest;
		if (roi_dest->width != mask[2] || roi_dest->height != mask[3])
			printf("rlucy_trim_mask: invalid dest size\n");
		if (roi_dest->x != mask[0] || roi_dest->y != mask[1])
			printf("rlucy_trim_mask: invalid dest origin\n");
	}
	
	if (roi_dest->width > roi_src->width || roi_dest->height > roi_src->height)
		printf("rlucy_trim_mask: roi_dest > roi_src\n");
	if (roi_dest->width+roi_dest->x > roi_src->width || roi_dest->height+roi_dest->y > roi_src->height)
		printf("rlucy_trim_mask: roi_dest+x > roi_src\n");
		
	// copy image
	for (int y = 0; y < roi_dest->height; y++)
	{
		const float *const src = img_src + (y + roi_dest->y) * roi_src->width * ch + roi_dest->x * ch;
		float *dest = img_trim + y * roi_dest->width * ch;
...
cleanup:
	...
	if (*img_dest == NULL) *img_dest = img_trim;
		...
		const int mode, float *img_dest, const dt_iop_roi_t *const roi_dest, double *time)
{
	...
	memset(img_dest, 0, roi_dest->width * roi_dest->height * ch * sizeof(float));
	
//  for (int c = 0; c < ch1; c++)
	{
	fftw3_convolve_t *fftw_conv = fftw3_convolve_init(image, roi_image->width, roi_image->height, ch, 
...
/*		fftw3_convolve(image, roi_image->width, roi_image->height, ch, c,
				...
				img_dest, roi_dest->width, roi_dest->height, 
				fftw3_mode, &(time[TIME_CONV_FFTW3_LOCK]));*/
  	fftw3_convolve(fftw_conv, c, &(time[TIME_CONV_FFTW3_LOCK]));
	}
	
	fftw3_convolve_free(fftw_conv, &(time[TIME_CONV_FFTW3_LOCK]));
	}
  
	time[TIME_CONVOLVE] += dt_get_wtime() - start;
	
	rlucy_check_nan(img_dest, roi_dest->width * roi_dest->height * ch, "rlucy_convolve fftw3");
	
}

#if 0
 void rlucy_convolve(const float *const image, const dt_iop_roi_t *const roi_image, const int ch, 
		const float *const kernel, const dt_iop_roi_t *const roi_kernel,
		const int mode, float *img_dest, const dt_iop_roi_t *const roi_dest, double *time)
{
		double start = dt_get_wtime();
...
//	const int max_tiles = 16;
	int threads = dt_get_num_threads() / 2;
	
	float **images = NULL;
	fftw3_convolve_t **fftw_conv = NULL;
	float *im_tiled = NULL;
	
	dt_iop_roi_t roi_tiled = *roi_image;
	...
	roi_tiled.width = (roi_image->width / threads) - 1;
	roi_tiled.height = roi_image->height;
	
	if (roi_tiled.width > roi_kernel->width)
	{
//		printf("roi_tiled\n");
		images = calloc(threads, sizeof(float*));
		fftw_conv = calloc(threads, sizeof(fftw3_convolve_t*));
		for (int i = 0; i < threads; i++)
		{
			im_tiled = dt_alloc_align(64, roi_image->width * roi_image->height * ch * sizeof(float));
			if (im_tiled == NULL) goto cleanup;
			
			dt_iop_roi_t roi_tiled_dest = {0};
			roi_tiled_dest = *roi_dest;
			
	  	if (mode == fftw3_convolve_full)
	  	{
	      roi_tiled_dest.height = (roi_tiled.height + roi_kernel->height-1);
	      roi_tiled_dest.width = (roi_tiled.width + roi_kernel->width-1);
	  	}
	  	else if (mode == fftw3_convolve_valid)
	  	{
			...
			images[i] = dt_alloc_align(64, roi_tiled_dest.width * roi_tiled_dest.height * ch * sizeof(float));
			if (images[i] == NULL) goto cleanup;
			
			memcpy(images[i], img_dest, roi_tiled_dest.width * roi_tiled_dest.height * ch * sizeof(float));
			
			fftw_conv[i] = fftw3_convolve_init(im_tiled, roi_tiled.width, roi_tiled.height, ch, 
					kernel, roi_kernel->width, roi_kernel->height, 
					images[i], roi_tiled_dest.width, roi_tiled_dest.height, 
					fftw3_mode, &(time[TIME_CONV_FFTW3_LOCK]));
		}
		
		memcpy(im_tiled, image, roi_image->width * roi_image->height * ch * sizeof(float));
	
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(threads, time, fftw_conv) schedule(static)
#endif
		for (int i = 0; i < threads; i++)
		{
/*			dt_iop_roi_t roi_tiled_dest = {0};
	  	...
	  	for (int c = 0; c < ch1; c++)
	  	fftw3_convolve(fftw_conv[i], c, &(time[TIME_CONV_FFTW3_LOCK]));
		}
	}
	else
	{
		_rlucy_convolve(image, roi_image, ch, 
				kernel, roi_kernel,
	...
	if (im_tiled) dt_free_align(im_tiled);
	if (images)
	{
		for (int i = 0; i < threads; i++)
		{
			if (images[i]) dt_free_align(images[i]);
			fftw3_convolve_free(fftw_conv[i], &(time[TIME_CONV_FFTW3_LOCK]));
		}
		free(images);
		free(fftw_conv);
	}
...
//	printf("rlucy_convolve took %0.04f sec\n", dt_get_wtime() - start);
	*time += dt_get_wtime() - start;
	...
	rlucy_check_nan(img_dest, roi_dest->width * roi_dest->height * ch, "rlucy_convolve_internal");
...
//	gradk = gradk + fftconvolve(np.rot90(u, 2), fftconvolve(u, psf, "valid") - image, "valid")
	// printf("rlucy_update_kernel 1\n");
	rlucy_check_nan(u, roi_u->width * roi_u->height * ch, "rlucy_update_kernel u");
	rlucy_check_nan(psf, roi_psf->width * roi_psf->height * ch, "rlucy_update_kernel psf");
	rlucy_convolve(u, roi_u, ch, psf, roi_psf, rlucy_convolve_valid, conv_1, roi_image, time);
	// printf("rlucy_update_kernel 11\n");
	rlucy_img_subtract(conv_1, roi_image, image, roi_image, conv_1, roi_image, ch, time);
	
	rlucy_img_rotate90(u, roi_u, u_rotate, roi_u, ch, 2, time);
	// printf("rlucy_update_kernel 2\n");
	rlucy_convolve(u_rotate, roi_u, ch, conv_1, roi_image, rlucy_convolve_valid, conv_2, roi_gradk, time);
	// printf("rlucy_update_kernel 21\n");
...
//	psf = psf - sh * gradk
	...
	for (int i = 0; i < roi_psf->width * roi_psf->height * ch; i++)
	{
		if (psf[i] < 0.f) psf[i] = 0.f;
	}
...
*/
 void rlucy_richardson_lucy(float *image, const dt_iop_roi_t *const roi_image, const int ch, 
			float *u, const dt_iop_roi_t *const roi_u,
			float *psf, const dt_iop_roi_t *const roi_psf,
		float *lambd, const int iterations, const int blind, const int *mask, double *time)
{
	dt_iop_roi_t roi_gradk = {0};
	dt_iop_roi_t roi_u_padded = {0};
	dt_iop_roi_t roi_masked_image = {0};
	dt_iop_roi_t roi_trim_u = {0};
	dt_iop_roi_t roi_masked_u = {0};
	
	float *gradk = NULL;
	float *u_padded = NULL;
	float *masked_image = NULL;
	float *trim_u = NULL;
	float *masked_u = NULL;
...
//	u = pad_image(image, pad_v, pad_h)
	rlucy_pad_image(u, roi_u, ch, pad_h, pad_v, &u_padded, &roi_u_padded);
	
	if (blind)
	{
		// printf("blind deconv begin\n");
		
//			gradk = np.zeros((MK, NK))
		roi_gradk = *roi_psf;
		gradk = dt_alloc_align(64, roi_gradk.width * roi_gradk.height * ch * sizeof(float));
		if (gradk == NULL) goto cleanup;
	
		memset(gradk, 0, roi_gradk.width * roi_gradk.height * ch * sizeof(float));
		
//    if mask != None:
//        masked_image = trim_mask(image, mask)
		if (mask != NULL)
		{
			rlucy_trim_mask(image, roi_image, ch, mask, &masked_image, &roi_masked_image);
		}
...
/*			with multiprocessing.Pool(processes=3) as pool:
			...
			// printf("blind deconv rlucy_update_image begin\n");
			rlucy_update_image(image, roi_image, ch, u_padded, &roi_u_padded, lambd, psf, roi_psf, time);
			// printf("blind deconv rlucy_update_image end\n");
...
//    masked_u = pad_image(trim_mask(u, mask), pad_v, pad_h)
			if (mask != NULL)
			{
				rlucy_trim_mask(u_padded, &roi_u_padded, ch, mask, &trim_u, &roi_trim_u);
				rlucy_pad_image(trim_u, &roi_trim_u, ch, pad_h, pad_v, &masked_u, &roi_masked_u);
				
				rlucy_check_nan(u_padded, roi_u_padded.width * roi_u_padded.height * ch, "rlucy_richardson_lucy u_padded");
				rlucy_check_nan(trim_u, roi_trim_u.width * roi_trim_u.height * ch, "rlucy_richardson_lucy trim_u");
				rlucy_check_nan(masked_u, roi_masked_u.width * roi_masked_u.height * ch, "rlucy_richardson_lucy masked_u");
			}
...
*/
			// printf("blind deconv rlucy_update_kernel begin\n");
			if (mask != NULL)
				rlucy_update_kernel(gradk, &roi_gradk, masked_u, &roi_masked_u, psf, roi_psf, masked_image, &roi_masked_image, ch, time);
			else
				rlucy_update_kernel(gradk, &roi_gradk, u_padded, &roi_u_padded, psf, roi_psf, image, roi_image, ch, time);
			// printf("blind deconv rlucy_update_kernel end\n");
...
/*			with multiprocessing.Pool(processes=3) as pool:
		...
		// printf("Non blind deconv begin\n");
		rlucy_loop_update_image(image, roi_image, ch, u_padded, &roi_u_padded, lambd, psf, roi_psf, iterations, time);
		// printf("Non blind deconv end\n");
	}
	
	rlucy_unpad_image(u, roi_u, ch, pad_h, pad_v, u_padded, &roi_u_padded);

cleanup:
	
	if (gradk) dt_free_align(gradk);
	if (u_padded) dt_free_align(u_padded);
	if (masked_image) dt_free_align(masked_image);
	if (trim_u) dt_free_align(trim_u);
	if (masked_u) dt_free_align(masked_u);
...
/*    images, kernels, lambdas = build_pyramid(image, blur_width, artifacts_damping)
    ...
    for i, k, l in zip(reversed(images), reversed(kernels), reversed(lambdas)):
		...
		u = (float*)dt_alloc_align(64, roi_u.width * roi_u.height * ch * sizeof(float));
		if (u == NULL) goto cleanup;
		memcpy(u, image, roi_u.width * roi_u.height * ch * sizeof(float));
	
		roi_kernel.width = roi_kernel.height = blur_width;

		psf = (float*)dt_alloc_align(64, roi_kernel.width * roi_kernel.height * ch * sizeof(float));
		if (psf == NULL) goto cleanup;

		rlucy_uniform_kernel(psf, &roi_kernel, ch);
		
		float lambd[4] = { artifacts_damping, artifacts_damping, artifacts_damping, artifacts_damping };
		
		rlucy_richardson_lucy(image, &roi_image, ch, u, &roi_u, psf, &roi_kernel, lambd, quality / 2, 1, NULL, time);
	...
	else
	{
		...
		if (blur_type == blur_type_gaussian)
		{
			roi_kernel.width = roi_kernel.height = blur_width;
	...
	//    psf = utils.gaussian_kernel(blur_strength, blur_width)
			rlucy_gaussian_kernel(psf, &roi_kernel, ch, blur_strength);
		}
	...
	//		psf = utils.kaiser_kernel(blur_strength, blur_width);
			rlucy_kaiser_kernel(psf, &roi_kernel, ch, blur_strength);
		}
		else
	...
	if (refine)
	{
		if (mask)
		{
			...
			// printf("Masked blind deconv begin\n");
			rlucy_richardson_lucy(image, &roi_image, ch, u, &roi_u, psf, &roi_kernel, lambd, iter_mask, 1, mask, time);
			// printf("Masked blind deconv done\n");
			
			if (iter_background != 0)
			{
					// u, psf = richardson_lucy(pic, u, psf, artifacts_damping, iter_background, blind=True)
				// printf("Unmasked blind deconv begin\n");
				rlucy_richardson_lucy(image, &roi_image, ch, u, &roi_u, psf, &roi_kernel, lambd, iter_background, 1, NULL, time);
				// printf("Unmasked blind deconv done\n");
			}
		}
		else
		{
			...
			// printf("Unmasked blind or myopic deconvolution begin\n");
			rlucy_richardson_lucy(image, &roi_image, ch, u, &roi_u, psf, &roi_kernel, lambd, quality, 1, NULL, time);
			// printf("Unmasked blind or myopic deconvolution done\n");
			
//			memcpy(image, u, roi_u.width * roi_u.height * ch * sizeof(float));
		}
    if (deblur_strength > 0)
    {
    	...
    	// printf("non-blind optimization begin\n");
    	rlucy_richardson_lucy(image, &roi_image, ch, u, &roi_u, psf, &roi_kernel, lambd, deblur_strength, 0, NULL, time);
    	// printf("non-blind optimization done\n");
    }
