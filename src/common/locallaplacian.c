...
#pragma omp parallel for default(none) schedule(static) collapse(2)
  ...
  for(int j = 1; j < ((ht - 1) & ~1); j++) // even ht: two px boundary. odd ht: one px.
    for(int i = 1; i < ((wd - 1) & ~1); i++) fine[j * wd + i] = ll_expand_gaussian(input, i, j, wd, ht);
  ll_fill_boundary2(fine, wd, ht);
}

#if defined(__SSE2__)
static inline void gauss_reduce_sse2(const float *const input, // fine input buffer
...
#pragma omp parallel for schedule(static) default(none)
    ...
    // init row pointers
    for(int k = 0; k < 5; k++) rows[k] = ringbuf + ((2 * j - 2 + k) % 5) * stride;
    ...
    const float *const row0 = rows[0], *const row1 = rows[1], *const row2 = rows[2], *const row3 = rows[3],
                       *const row4 = rows[4];
...
static inline float *ll_pad_input(const float *const input, const int wd, const int ht, const int max_supp,
                                  int *wd2, int *ht2, local_laplacian_boundary_t *b)
{
  const int stride = 4;
  *wd2 = 2 * max_supp + wd;
  *ht2 = 2 * max_supp + ht;
...
#pragma omp parallel for schedule(dynamic) default(none) shared(wd2, ht2, b) collapse(2)
    ...
    for(int j = max_supp; j < *ht2 - max_supp; j++)
      for(int i = 0; i < max_supp; i++) LL_FILL(input[stride * wd * (j - max_supp)] * 0.01f);
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) default(none) shared(wd2, ht2, b) collapse(2)
#endif // right border
    for(int j = max_supp; j < *ht2 - max_supp; j++)
      for(int i = wd + max_supp; i < *wd2; i++) LL_FILL(input[stride * ((j - max_supp) * wd + wd - 1)] * 0.01f);
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) default(none) shared(wd2, ht2, b) collapse(2)
#endif // top border
    for(int j = 0; j < max_supp; j++)
      for(int i = 0; i < *wd2; i++) LL_FILL(out[*wd2 * max_supp + i]);
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) default(none) shared(wd2, ht2, b) collapse(2)
#endif // bottom border
    for(int j = max_supp + ht; j < *ht2; j++)
      for(int i = 0; i < *wd2; i++) LL_FILL(out[*wd2 * (max_supp + ht - 1) + i]);
#undef LL_FILL
  }
  else
  { // pad by replication:
...
{
  const float c
      = ll_expand_gaussian(coarse, CLAMPS(i, 1, ((wd - 1) & ~1) - 1), CLAMPS(j, 1, ((ht - 1) & ~1) - 1), wd, ht);
  return fine[j * wd + i] - c;
}

static inline float curve_scalar(const float x, const float g, const float sigma, const float shadows,
...
#if defined(__SSE2__)
static inline __m128 curve_vec4(const __m128 x, const __m128 g, const __m128 sigma, const __m128 shadows,
                                const __m128 highlights, const __m128 clarity)
{
  ...
  const __m128 t = _mm_min_ps(one, _mm_max_ps(_mm_setzero_ps(), _mm_div_ps(c, _mm_mul_ps(two, ssigma))));
  ...
  const __m128 vmid = _mm_add_ps(g, _mm_add_ps(_mm_mul_ps(_mm_mul_ps(ssigma, two), _mm_mul_ps(mt, t)),
...
// sse (4-wide)
void apply_curve_sse2(float *const out, const float *const in, const uint32_t w, const uint32_t h,
...
#pragma omp parallel for default(none) schedule(dynamic)
  ...
  for(uint32_t j = padding; j < h - padding; j++)
  {
    ...
    for(; out2 < beg; out2++, in2++) *out2 = curve_scalar(*in2, g, sigma, shadows, highlights, clarity);
    for(; out2 < end; out2 += 4, in2 += 4)
      _mm_stream_ps(out2, curve_vec4(_mm_load_ps(in2), g4, sig4, shd4, hil4, clr4));
    for(; out2 < fin; out2++, in2++) *out2 = curve_scalar(*in2, g, sigma, shadows, highlights, clarity);
    out2 = out + j * w;
...
#define num_gamma 6
  ...
  for(int l = 1; l <= last_level; l++) padded[l] = dt_alloc_align(16, sizeof(float) * dl(w, l) * dl(h, l));

  // allocate pyramid pointers for output
  float *output[max_levels] = { 0 };
  for(int l = 0; l <= last_level; l++) output[l] = dt_alloc_align(16, sizeof(float) * dl(w, l) * dl(h, l));

// create gauss pyramid of padded input, write coarse directly to output
#if defined(__SSE2__)
  if(use_sse2)
  {
    for(int l = 1; l < last_level; l++) gauss_reduce_sse2(padded[l - 1], padded[l], dl(w, l - 1), dl(h, l - 1));
    gauss_reduce_sse2(padded[last_level - 1], output[last_level], dl(w, last_level - 1), dl(h, last_level - 1));
  }
  else
#endif
  {
    for(int l = 1; l < last_level; l++) gauss_reduce(padded[l - 1], padded[l], dl(w, l - 1), dl(h, l - 1));
  ...
  for(int k = 0; k < num_gamma; k++)
    for(int l = 0; l <= last_level; l++) buf[k][l] = dt_alloc_align(16, sizeof(float) * dl(w, l) * dl(h, l));
