...
void init(dt_iop_module_t *module) {
    ...
    tmp.blur_type = rlucy_blur_type_auto;
    tmp.blur_width = 5;
    tmp.quality = 1;
...
void gui_init(struct dt_iop_module_t *self) {
    ...
    g_object_set(g->sl_quality, "tooltip-text",
                 _("number of iterations to perform. It should be around the size of the blur"), (char *) NULL);
    ...
    g_object_set(g->sl_ringing_factor, "tooltip-text",
                 _("increase this factor to speed-up the convergence, decrease it if ringing grows out of control"),
                 (char *) NULL);
    ...
    g_object_set(g->sl_blur_width, "tooltip-text", _("the width of the blur in px - must be an odd integer"),
                 (char *) NULL);
    g_signal_connect(G_OBJECT(g->sl_blur_width), "value-changed", G_CALLBACK(rlucy_blur_width_callback), self);


    g->chk_refine = gtk_check_button_new_with_label("refine the initial blur estimation");
    g_object_set(G_OBJECT(g->chk_refine), "tooltip-text",
                 _("decide if the blur kernel should be refined through myopic deconvolution"), (char *) NULL);
    ...
    g_object_set(G_OBJECT(g->chk_use_mask), "tooltip-text", _("select the area of interest to force the focus"),
                 (char *) NULL);
    ...
    g->sl_noise_reduction_factor = dt_bauhaus_slider_new_with_range(self, 500, 30000, 100, p->noise_reduction_factor,
                                                                    0);
    dt_bauhaus_widget_set_label(g->sl_noise_reduction_factor, _("noise tolerance factor"), _("noise tolerance factor"));
    g_object_set(g->sl_noise_reduction_factor, "tooltip-text",
                 _("an higher value may lead to a sharper result but may increase the noise as well"), (char *) NULL);
...
void rlucy_process_internal(struct dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, const void *const ivoid,
                            void *const ovoid,
    ...
    if (cst == iop_cs_rgb) {
        rlucy_deblur_module((float *) ivoid,
                            blur_type,
                            blur_width,
