...
} dt_iop_dt_gmic_exp_params_t;

typedef struct dt_iop_dt_gmic_exp_gui_data_t
{
	int image_requested;
	int image_locked;
...
} dt_iop_dt_gmic_exp_gui_data_t;

typedef struct dt_iop_dt_gmic_exp_params_t dt_iop_dt_gmic_exp_data_t;

typedef struct dt_iop_dt_gmic_exp_global_data_t
{
	gmic_qt_lib_t * gmic_gd;
} dt_iop_dt_gmic_exp_global_data_t;


const char *name()
{
  return _("dt_gmic_exp_eh");
}
...
{
  dt_iop_dt_gmic_exp_gui_data_t *g = (dt_iop_dt_gmic_exp_gui_data_t *)self->gui_data;
  dt_iop_dt_gmic_exp_params_t *p = (dt_iop_dt_gmic_exp_params_t *)self->params;
  dt_iop_dt_gmic_exp_global_data_t *gd = (dt_iop_dt_gmic_exp_global_data_t *)self->data;
...
static void update_filter_labels(gmic_filter_definition_t *filter_definition, dt_iop_dt_gmic_exp_gui_data_t *g)
{
	const char *blank = "";
...
// update values on already created gui controls from values in dt_iop_dt_gmic_exp_params_t->filter_data
	...
	if (filter_definition)
	{
		...
		if (filter_definition->params)
		{
			...
			for (int num_p = 0; num_p < filter_definition->num_params; num_p++, params++)
			{
				...
				{
					  GdkRGBA color = (GdkRGBA){.red = params->rgb_current_value[0] * (1.f / 255.f), 
					    .green = params->rgb_current_value[1] * (1.f / 255.f), 
					    .blue = params->rgb_current_value[2] * (1.f / 255.f), 
					    .alpha = 1.0 };
...
static void call_dt_gmic_exp_qt_callback(GtkWidget *widget, dt_iop_module_t *self)
{
  dt_iop_dt_gmic_exp_gui_data_t *g = (dt_iop_dt_gmic_exp_gui_data_t *)self->gui_data;
  dt_iop_dt_gmic_exp_params_t *p = (dt_iop_dt_gmic_exp_params_t *)self->params;
  dt_iop_dt_gmic_exp_global_data_t *gd = (dt_iop_dt_gmic_exp_global_data_t *)self->data;
...
{
  dt_iop_dt_gmic_exp_params_t *p = (dt_iop_dt_gmic_exp_params_t *)p1;
  dt_iop_dt_gmic_exp_data_t *d = (dt_iop_dt_gmic_exp_data_t *)piece->data;
  memcpy(d, p, sizeof(dt_iop_dt_gmic_exp_params_t));
}

void init_pipe(struct dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
  piece->data = malloc(sizeof(dt_iop_dt_gmic_exp_data_t));
...
{
  module->params = calloc(1, sizeof(dt_iop_dt_gmic_exp_params_t));
  module->default_params = calloc(1, sizeof(dt_iop_dt_gmic_exp_params_t));
  module->default_enabled = 0;
  module->params_size = sizeof(dt_iop_dt_gmic_exp_params_t);
  module->gui_data = NULL;
//  module->priorityx = 955; // module order created by iop_dependencies.py, do not edit! // from borders.c
  module->priority = 147; // module order created by iop_dependencies.py, do not edit! // from tonemap (before exposure)
//  module->priorityx = 367; // module order created by iop_dependencies.py, do not edit! // from colorreconstruction
...
{
  dt_iop_dt_gmic_exp_global_data_t *gd = (dt_iop_dt_gmic_exp_global_data_t *)malloc(sizeof(dt_iop_dt_gmic_exp_global_data_t));
  module->data = gd;
...
{
  dt_iop_dt_gmic_exp_global_data_t *gd = (dt_iop_dt_gmic_exp_global_data_t *)module->data;
...
{
  self->gui_data = malloc(sizeof(dt_iop_dt_gmic_exp_gui_data_t));
  dt_iop_dt_gmic_exp_gui_data_t *g = (dt_iop_dt_gmic_exp_gui_data_t *)self->gui_data;
  ...
  g_signal_connect(G_OBJECT(widget), "clicked", G_CALLBACK(call_dt_gmic_exp_qt_callback), self);
...
{
  dt_iop_dt_gmic_exp_params_t tmp = { 0 };
  
  memcpy(self->params, &tmp, sizeof(dt_iop_dt_gmic_exp_params_t));
  memcpy(self->default_params, &tmp, sizeof(dt_iop_dt_gmic_exp_params_t));
...
static void copy_image_from_gmic(float *image, float *gmic_image, const dt_iop_roi_t *const roi_in, const int ch_in, const int ch_gmic, const dt_iop_colorspace_type_t cst, const int pixelpipe_pos)
{
  ...
  if (pixelpipe_pos == dt_iop_gmic_pipepos_rgb /*|| pixelpipe_pos == dt_iop_gmic_pipepos_linear_rgb*/) // after colorout
  {
    ...
    if (ch_gmic == 1 || ch_gmic == 2)
    {
      ...
      {
        out[0] = out[1] = out[2] = in[0] * (1.f / 255.f);
      }
    }
    else
    {
      for(int j = 0; j < roi_in->width*roi_in->height; j++, out += ch_in, in += ch_in)
      {
        out[0] = in[0] * (1.f / 255.f);
        out[1] = in[1] * (1.f / 255.f);
        out[2] = in[2] * (1.f / 255.f);
      }
    }
  }
  else if (pixelpipe_pos == dt_iop_gmic_pipepos_lab) // after colorin
    ...
    if (ch_gmic == 1 || ch_gmic == 2)
    {
      for(int j = 0; j < roi_in->width*roi_in->height; j++, out += ch_in, in += ch_in)
      {
        // transform the result back to Lab
      	...
      	rgb[1] = rgb[2] = rgb[0] = in[0] * (1.f / 255.f);
    ...
    else
    {
      for(int j = 0; j < roi_in->width*roi_in->height; j++, out += ch_in, in += ch_in)
      {
        // transform the result back to Lab
      	...
      	for(int c = 0; c < 3; c++) rgb[c] = in[c] * (1.f / 255.f);
...
{
  dt_iop_dt_gmic_exp_data_t *p = (dt_iop_dt_gmic_exp_data_t *)piece->data;
  dt_iop_dt_gmic_exp_gui_data_t *g = (dt_iop_dt_gmic_exp_gui_data_t *)self->gui_data;
  dt_iop_dt_gmic_exp_global_data_t *gd = (dt_iop_dt_gmic_exp_global_data_t *)self->data;
