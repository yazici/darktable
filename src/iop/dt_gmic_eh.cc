...
typedef enum dt_iop_dt_gmic_control_type_t
{
	dt_iop_dt_gmic_control_sl = 0,
	dt_iop_dt_gmic_control_cmb = 1,
	dt_iop_dt_gmic_control_lbl = 2,
	dt_iop_dt_gmic_control_btn = 3,
	dt_iop_dt_gmic_control_colorbtn = 4,
	dt_iop_dt_gmic_control_sep = 5,
	dt_iop_dt_gmic_control_txt = 6,
	dt_iop_dt_gmic_control_link = 7,
	dt_iop_dt_gmic_control_box = 8,
	dt_iop_dt_gmic_control_chk = 9,
} dt_iop_dt_gmic_control_type_t;

typedef enum dt_iop_dt_gmic_pixelpipepos_t
{
	dt_iop_dt_gmic_rgb = 0,
	dt_iop_dt_gmic_lab = 1,
	dt_iop_dt_gmic_linear_rgb = 2,
		...
		if (filter_definition->params)
		{
			...
			for (int num_p = 0; num_p < filter_definition->num_params; num_p++, params++)
			{
				...
				if (params->param_type == gmic_param_int || params->param_type == gmic_param_float)
				{
					...
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_sl);
					if (w_index >= 0)
					{
						g->sl_widget[w_index] = 
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_chk);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_cmb);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_colorbtn);
					int w_box_index = get_control_index(g, dt_iop_dt_gmic_control_box);
					int w_lbl_index = get_control_index(g, dt_iop_dt_gmic_control_lbl);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_sep);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_lbl);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_link);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_txt);
					if (w_index >= 0)
					{
				...
				{
					int w_index = get_control_index(g, dt_iop_dt_gmic_control_btn);
					if (w_index >= 0)
					{
						char *str = (params->str_current_value) ? params->str_current_value: 
												(params->str_default_value) ? params->str_default_value: params->param_name;
...
static void copy_image_to_gmic(float *image, float *gmic_image, const dt_iop_roi_t *const roi_in, const int ch_in, const int ch_gmic, const dt_iop_colorspace_type_t cst, const int pixelpipe_pos)
{
  ...
  if (pixelpipe_pos == dt_iop_dt_gmic_rgb /*|| pixelpipe_pos == dt_iop_dt_gmic_linear_rgb*/) // after colorout
  {
  ...
  else if (pixelpipe_pos == dt_iop_dt_gmic_lab) // after colorin
  {
  ...
  else if (pixelpipe_pos == dt_iop_dt_gmic_linear_rgb) // after demosaic
  {
    if (cst != iop_cs_rgb) return;
