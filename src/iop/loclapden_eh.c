...
DT_MODULE_INTROSPECTION(1, dt_iop_loclapden_params_t)

#define LOCLAP_MAX_SCALES 8

typedef struct dt_iop_loclapden_params_t
{
  float alpha;
  float beta;
  float scales[LOCLAP_MAX_SCALES];
} dt_iop_loclapden_params_t;

typedef struct dt_iop_loclapden_gui_data_t
{
  GtkWidget *sl_alpha;
  GtkWidget *sl_beta;
  GtkWidget *sl_scales[LOCLAP_MAX_SCALES];
} dt_iop_loclapden_gui_data_t;

typedef struct dt_iop_loclapden_params_t dt_iop_loclapden_data_t;

const char *name()
{
  return _("loclapden_eh");
}
...
{
  memcpy(piece->data, params, sizeof(dt_iop_loclapden_params_t));
}

void init_pipe(struct dt_iop_module_t *self, dt_dev_pixelpipe_t *pipe, dt_dev_pixelpipe_iop_t *piece)
{
  piece->data = malloc(sizeof(dt_iop_loclapden_data_t));
...
{
  dt_iop_loclapden_gui_data_t *g = (dt_iop_loclapden_gui_data_t *)self->gui_data;
  dt_iop_loclapden_params_t *p = (dt_iop_loclapden_params_t *)self->params;
...
{
	module->data = NULL;
  module->params = calloc(1, sizeof(dt_iop_loclapden_params_t));
  module->default_params = calloc(1, sizeof(dt_iop_loclapden_params_t));
...
//  module->priority = 671; // from tonecurve
  ...
  module->params_size = sizeof(dt_iop_loclapden_params_t);
  module->gui_data = NULL;
  
  dt_iop_loclapden_params_t tmp = {0};
  
  memcpy(module->params, &tmp, sizeof(dt_iop_loclapden_params_t));
  memcpy(module->default_params, &tmp, sizeof(dt_iop_loclapden_params_t));
...
{
  self->gui_data = malloc(sizeof(dt_iop_loclapden_gui_data_t));
  dt_iop_loclapden_gui_data_t *g = (dt_iop_loclapden_gui_data_t *)self->gui_data;
...
{
  const dt_iop_loclapden_data_t *const p = (const dt_iop_loclapden_data_t *const)piece->data;
...
*/
  ...
  image_L = dt_alloc_align(64, roi_in->width * roi_in->height * sizeof(float));
	...
	loclapden_LocalLaplacianDen(image_L, image_L, roi_out->width, roi_out->height, p->alpha, p->beta, p->scales, (roi_in->scale / piece->iscale), use_sse);
