...
dt_liquify_layer_t dt_liquify_layers[] = {
  ...
  {
   DT_LIQUIFY_LAYER_HARDNESSPOINT1, GREY, LGREY, 1.0,
   DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED,
  },
  {
   DT_LIQUIFY_LAYER_HARDNESSPOINT2, GREY, LGREY, 1.0,
   DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED,
  },
  {
  ...
  {
   DT_LIQUIFY_LAYER_HARDNESSPOINT1, GREY, LGREY, 1.0,
   DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED | DT_LIQUIFY_LAYER_FLAG_HIT_TEST,
  },
  {
   DT_LIQUIFY_LAYER_HARDNESSPOINT2, GREY, LGREY, 1.0,
   DT_LIQUIFY_LAYER_FLAG_NODE_TOOL | DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED | DT_LIQUIFY_LAYER_FLAG_HIT_TEST,
  ...
  {
   DT_LIQUIFY_LAYER_STRENGTHPOINT, GREY, LGREY, 1.0,
                               ...
                               dt_liquify_path_data_t *new)
{
  ...
  if(this->header.prev != -1) p->nodes[this->header.prev].header.next = new->header.idx;
...
static void node_gc(dt_iop_liquify_params_t *p)
{
  ...
  for(last = MAX_NODES - 1; last > 0; last--)
    if(p->nodes[last].header.type != DT_LIQUIFY_PATH_INVALIDATED) break;
  int k = 0;
  ...
  for(int k = last + 1; k < MAX_NODES; k++) p->nodes[k].header.type = DT_LIQUIFY_PATH_INVALIDATED;
}
...
static void node_delete(dt_iop_liquify_params_t *p, dt_liquify_path_data_t *this)
{
  ...
  else if(prev)
  {
    ...
    if(next) next->header.prev = prev->header.idx;
  }
...
static void _distort_paths(const struct dt_iop_module_t *module, const distort_params_t *params,
                           const dt_iop_liquify_params_t *p)
{
  ...
  for(int k = 0; k < MAX_NODES; k++)
  {
    ...
    if(data->header.type == DT_LIQUIFY_PATH_INVALIDATED) break;
...
static void distort_paths_raw_to_piece(const struct dt_iop_module_t *module, dt_dev_pixelpipe_t *pipe,
...
static void mix_warps(dt_liquify_warp_t *result, const dt_liquify_warp_t *warp1, const dt_liquify_warp_t *warp2,
                      const complex float pt, const float t)
{
...
static void interpolate_cubic_bezier(const float complex p0, const float complex p1, const float complex p2,
                                     const float complex p3, float complex buffer[], const int n)
{
...
{
  float length = 0.0;
  for(int i = 1; i < n_points; i++) length += cabs(points[i - 1] - points[i]);
  return length;
}
...
static const float complex point_at_arc_length(const float complex points[], const int n_points,
...
static float *build_lookup_table(const int distance, const float control1, const float control2)
{
  ...
  *ptr++ = 1.0;
  ...
  {
    x += step;
    while(creal(*cptr) < x && cptr < cptr_end) cptr++;
...
static void compute_round_stamp_extent(cairo_rectangle_int_t *stamp_extent, const dt_liquify_warp_t *warp)
{
...
static void build_round_stamp(float complex **pstamp, cairo_rectangle_int_t *stamp_extent,
                              const dt_liquify_warp_t *warp)
{
  ...
  strength = (warp->status & DT_LIQUIFY_STATUS_INTERPOLATED) ? (strength * STAMP_RELOCATION) : strength;
  const float abs_strength = cabs(strength);

  float complex *stamp = malloc(sizeof(float complex) * stamp_extent->width * stamp_extent->height);
  ...
  for(int y = 0; y <= iradius; y++)
  {
    for(int x = y; x <= iradius; x++)
    {
      ...
      switch(warp->type)
      {
        ...
        default:
          *o1 = *o2 = *o3 = *o4 = *o5 = *o6 = *o7 = *o8 = strength *lookup_table[idist];
          break;
      }
    }
  next_row:
...
static void add_to_global_distortion_map(float complex *global_map, const cairo_rectangle_int_t *global_map_extent,
                                         const dt_liquify_warp_t *warp, const float complex *stamp,
...
#pragma omp parallel for schedule(static) default(shared)
  ...
  for(int y = cmmext.y; y < cmmext.y + cmmext.height; y++)
  {
    ...
    float complex *destrow = global_map + ((y - global_map_extent->y) * global_map_extent->width);
...
static void apply_global_distortion_map(struct dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
                                        const float *in, float *out, const dt_iop_roi_t *roi_in,
                                        const dt_iop_roi_t *roi_out, const float complex *map,
...
#pragma omp parallel for schedule(static) default(shared)
  ...
  for(int y = extent->y; y < extent->y + extent->height; y++)
  {
    ...
    if(y >= roi_out->y && y < roi_out->y + roi_out->height)
    {
      ...
      float *out_sample = out + ((y - roi_out->y) * roi_out->width + extent->x - roi_out->x) * ch;
...
static void _get_map_extent(const dt_iop_roi_t *roi_out, GList *interpolated, cairo_rectangle_int_t *map_extent)
{
...
static float complex *create_global_distortion_map(const cairo_rectangle_int_t *map_extent, GList *interpolated,
                                                   gboolean inverted)
{
...
static float complex *build_global_distortion_map(struct dt_iop_module_t *module,
                                                  ...
                                                  const dt_iop_roi_t *roi_out, cairo_rectangle_int_t *map_extent)
{
  // copy params
...
void modify_roi_out(struct dt_iop_module_t *module, struct dt_dev_pixelpipe_iop_t *piece, dt_iop_roi_t *roi_out,
...
void modify_roi_in(struct dt_iop_module_t *module, struct dt_dev_pixelpipe_iop_t *piece,
                   const dt_iop_roi_t *roi_out, dt_iop_roi_t *roi_in)
{
  ...
  *roi_in = *roi_out;
  ...
  cairo_rectangle_int_t pipe_rect = { 0, 0, lroundf((double)piece->buf_in.width * roi_in->scale),
...
static int _distort_xtransform(dt_iop_module_t *self, dt_dev_pixelpipe_iop_t *piece, float *points,
                               size_t points_count, gboolean inverted)
{
  ...
  if(extent.width != 0 && extent.height != 0)
  {
    ...
    // apply distortion to all points (this is a simple displacement given by a vector at this same point in the
    // map)
...
void process(struct dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece, const void *const in, void *const out,
...
#pragma omp parallel for schedule(static) default(none)
  ...
  for(int i = 0; i < roi_out->height; i++)
  {
    ...
    const float *srcrow = (float *)in
                          + (size_t)ch * (roi_in->width * (i + roi_out->y - roi_in->y) + roi_out->x - roi_in->x);
...
static cl_int_t apply_global_distortion_map_cl(struct dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece,
                                               ...
                                               const float complex *map, const cairo_rectangle_int_t *map_extent)
{
  ...
  switch(interpolation->id)
  {
    ...
    case DT_INTERPOLATION_BICUBIC:
      ...
      for(int i = 0; i <= kdesc.size * kdesc.resolution; ++i) k[i] = bicubic(0.5f, (float)i / kdesc.resolution);
      break;
    case DT_INTERPOLATION_LANCZOS2:
      kdesc.size = 2;
      k = malloc((kdesc.size * kdesc.resolution + 1) * sizeof(float));
      for(int i = 0; i <= kdesc.size * kdesc.resolution; ++i) k[i] = lanczos(2, (float)i / kdesc.resolution);
      break;
    case DT_INTERPOLATION_LANCZOS3:
      kdesc.size = 3;
      k = malloc((kdesc.size * kdesc.resolution + 1) * sizeof(float));
      for(int i = 0; i <= kdesc.size * kdesc.resolution; ++i) k[i] = lanczos(3, (float)i / kdesc.resolution);
      break;
    default:
      return FALSE;
  }

  cl_mem_t dev_roi_in = dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), (void *)roi_in);

  cl_mem_t dev_roi_out = dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_iop_roi_t), (void *)roi_out);
  ...
  cl_mem_t dev_map_extent
      = dt_opencl_copy_host_to_device_constant(devid, sizeof(cairo_rectangle_int_t), (void *)map_extent);

  cl_mem_t dev_kdesc
      = dt_opencl_copy_host_to_device_constant(devid, sizeof(dt_liquify_kernel_descriptor_t), (void *)&kdesc);

  cl_mem_t dev_kernel = dt_opencl_copy_host_to_device_constant(
      devid, (kdesc.size * kdesc.resolution + 1) * sizeof(float), (void *)k);

  if(dev_roi_in == NULL || dev_roi_out == NULL || dev_map == NULL || dev_map_extent == NULL || dev_kdesc == NULL
     || dev_kernel == NULL)
    goto error;
...
int process_cl(struct dt_iop_module_t *module, dt_dev_pixelpipe_iop_t *piece, const cl_mem_t dev_in,
...
void commit_params(struct dt_iop_module_t *module, dt_iop_params_t *params, dt_dev_pixelpipe_t *pipe,
...
static gboolean detect_drag(const dt_iop_liquify_gui_data_t *g, const double scale, const float complex pt)
{
  ...
  return g->last_button1_pressed_pos != -1.0 && cabs(pt - g->last_button1_pressed_pos) >= GET_UI_WIDTH(MIN_DRAG);
}
...
static void update_warp_count(const dt_iop_liquify_gui_data_t *g)
{
  ...
  for(int k = 0; k < MAX_NODES; k++)
    ...
    {
      node++;
      if(g->params.nodes[k].header.type == DT_LIQUIFY_PATH_MOVE_TO_V1) warp++;
    }
  char str[10];
...
static GList *interpolate_paths(dt_iop_liquify_params_t *p)
{
  ...
  for(int k = 0; k < MAX_NODES; k++)
  {
    ...
    if(data->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
    {
      ...
      interpolate_cubic_bezier(*p1, data->node.ctrl1, data->node.ctrl2, *p2, buffer, INTERPOLATION_POINTS);
...
#define STROKE_TEST                                                                                               \
  if(do_hit_test)                                                                                                 \
  {                                                                                                               \
    if(cairo_in_stroke(cr, creal(*pt), cimag(*pt))) goto hit;                                                     \
    continue;                                                                                                     \
  }
...
#define FILL_TEST                                                                                                 \
  if(do_hit_test)                                                                                                 \
  {                                                                                                               \
    if(cairo_in_fill(cr, creal(*pt), cimag(*pt)) || cairo_in_stroke(cr, creal(*pt), cimag(*pt))) goto hit;        \
    continue;                                                                                                     \
  }
...
static dt_liquify_hit_t _draw_paths(dt_iop_module_t *module, cairo_t *cr, const float scale,
                                    dt_iop_liquify_params_t *p, GList *layers, const float complex *pt)
{
  ...
  for(GList *l = layers; l != NULL; l = l->next)
  {
    ...
    if(do_hit_test && ((dt_liquify_layers[layer].flags & DT_LIQUIFY_LAYER_FLAG_HIT_TEST) == 0)) continue;

    hit.layer = layer;

    if(dt_liquify_layers[layer].opacity < 1.0) cairo_push_group(cr);
    ...
    for(int k = 0; k < MAX_NODES; k++)
    {
      ...
      if(p->nodes[k].header.type == DT_LIQUIFY_PATH_INVALIDATED) break;

      hit.elem = data;

      if((dt_liquify_layers[layer].flags & DT_LIQUIFY_LAYER_FLAG_NODE_SELECTED) && !data->header.selected)
        continue;
      ...
      if(data->header.selected == layer) fg_color = DT_LIQUIFY_COLOR_SELECTED;

      if(data->header.hovered == dt_liquify_layers[layer].hover_master) fg_color = DT_LIQUIFY_COLOR_HOVER;

      cairo_new_path(cr);

      const float complex point = data->warp.point;

      if(data->header.type == DT_LIQUIFY_PATH_MOVE_TO_V1) cairo_move_to(cr, creal(point), cimag(point));
      ...
      if(layer == DT_LIQUIFY_LAYER_WARPS)
      {
        ...
        for(GList *i = interpolated; i != NULL; i = i->next)
        {
          ...
          draw_triangle(cr, pwarp->strength, carg(pwarp->strength - pwarp->point) + rot,
      ...
      {
        if((data->header.type == DT_LIQUIFY_PATH_LINE_TO_V1) || (data->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1))
        {
          assert(prev);
          cairo_move_to(cr, creal(prev->warp.point), cimag(prev->warp.point));
          if(data->header.type == DT_LIQUIFY_PATH_LINE_TO_V1) cairo_line_to(cr, creal(point), cimag(point));
          if(data->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
          {
            cairo_curve_to(cr, creal(data->node.ctrl1), cimag(data->node.ctrl1), creal(data->node.ctrl2),
                           cimag(data->node.ctrl2), creal(point), cimag(point));
          }
          THICKLINE;
          FG_COLOR;
          STROKE_TEST;
      ...
      {
        if(data->header.type == DT_LIQUIFY_PATH_MOVE_TO_V1 || data->header.type == DT_LIQUIFY_PATH_LINE_TO_V1
      ...
      if(data->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1)
      {
        ...
        if(layer == DT_LIQUIFY_LAYER_CTRLPOINT2_HANDLE && data->header.node_type != DT_LIQUIFY_NODE_TYPE_AUTOSMOOTH)
        {
          THINLINE;
          FG_COLOR;
        ...
        if(layer == DT_LIQUIFY_LAYER_CTRLPOINT2 && data->header.node_type != DT_LIQUIFY_NODE_TYPE_AUTOSMOOTH)
        {
          THINLINE;
          BG_COLOR;
      ...
      {
        draw_triangle(cr, cmix(point, warp->radius, warp->control1), carg(warp->radius - point),
      ...
      {
        draw_triangle(cr, cmix(point, warp->radius, warp->control2), carg(-(warp->radius - point)),
                      GET_UI_WIDTH(GIZMO_SMALL));
        THINLINE;
        BG_COLOR;
        FILL_TEST;
      ...
      if(layer == DT_LIQUIFY_LAYER_STRENGTHPOINT_HANDLE)
      {
        ...
        {
          float complex pt = cmix(point, warp->strength,
                                  1.0 - 0.5 * (GET_UI_WIDTH(GIZMO_SMALL) / cabs(warp->strength - point)));
      ...
      if(layer == DT_LIQUIFY_LAYER_STRENGTHPOINT)
      {
        ...
        draw_triangle(cr, warp->strength, carg(warp->strength - warp->point) + rot, GET_UI_WIDTH(GIZMO_SMALL));
        THINLINE;
        BG_COLOR;
        FILL_TEST;
        cairo_fill_preserve(cr);
...
static dt_liquify_hit_t hit_test_paths(struct dt_iop_module_t *module, float scale, cairo_t *cr,
                                       dt_iop_liquify_params_t *params, float complex pt)
{
  dt_liquify_hit_t hit = NOWHERE;
  GList *layers = NULL;
...
static void casteljau(const float complex *p0, float complex *p1, float complex *p2, float complex *p3,
                      const float t)
{
...
static float find_nearest_on_curve_t(const float complex p0, const float complex p1, const float complex p2,
                                     const float complex p3, const float complex x, const int n)
{
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    const float complex ip = t1 * t1 * t1 * p0 + 3 * t1 * t1 * t * p1 + 3 * t1 * t * t * p2 + t * t * t * p3;
...
#undef ABCD
  ...
  c1[n - 1] = d[n - 1] / b[n - 1];
  for(int i = n - 2; i >= 0; i--) c1[i] = (d[i] - c[i] * c1[i + 1]) / b[i];
...
static void smooth_paths_linsys(dt_iop_liquify_params_t *params)
{
  for(int k = 0; k < MAX_NODES; k++)
  {
    if(params->nodes[k].header.type == DT_LIQUIFY_PATH_INVALIDATED) break;

    if(params->nodes[k].header.prev != -1) continue;
...
void gui_post_expose(struct dt_iop_module_t *module, cairo_t *cr, int32_t width, int32_t height, int32_t pointerx,
                     int32_t pointery)
{
...
int mouse_moved(struct dt_iop_module_t *module, double x, double y, double pressure, int which)
{
  ...
  if(!is_dragging(g))
  {
    ...
    {
      if(hit.elem) hit.elem->header.hovered = hit.layer;
      if(last_hovered) last_hovered->header.hovered = 0;
    ...
    switch(g->dragging.layer)
    {
      case DT_LIQUIFY_LAYER_CENTERPOINT:
        switch(d->header.type)
        {
          ...
          case DT_LIQUIFY_PATH_MOVE_TO_V1:
          case DT_LIQUIFY_PATH_LINE_TO_V1:
            if(n && n->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1) n->node.ctrl1 += pt - d->warp.point;
            if(p && p->header.type == DT_LIQUIFY_PATH_CURVE_TO_V1) p->node.ctrl2 += pt - d->warp.point;
              ...
              switch(p->header.node_type)
              {
                case DT_LIQUIFY_NODE_TYPE_SMOOTH:
                  p->node.ctrl2 = p->warp.point
                                  + cabs(p->warp.point - p->node.ctrl2) * cexp(carg(p->warp.point - pt) * I);
                  break;
                case DT_LIQUIFY_NODE_TYPE_SYMMETRICAL:
              ...
              switch(d->header.node_type)
              {
                case DT_LIQUIFY_NODE_TYPE_SMOOTH:
                  n->node.ctrl1 = d->warp.point
                                  + cabs(d->warp.point - n->node.ctrl1) * cexp(carg(d->warp.point - pt) * I);
                  break;
                case DT_LIQUIFY_NODE_TYPE_SYMMETRICAL:
...
int button_pressed(struct dt_iop_module_t *module, double x, double y, double pressure, int which, int type,
                   uint32_t state)
{
  dt_iop_liquify_gui_data_t *g = (dt_iop_liquify_gui_data_t *)module->gui_data;
  int handled = 0;
  ...
  if(which == 1) g->last_button1_pressed_pos = pt;
  ...
  if(which == 1 && gtk_toggle_button_get_active(g->btn_point_tool))
  {
    ...
    if(!g->temp) goto done;
    g->temp->warp.radius
        = pt + (dt_conf_key_exists(CONF_RADIUS) ? dt_conf_get_float(CONF_RADIUS) : GET_UI_WIDTH(DEFAULT_RADIUS));
  ...
  if(which == 1
     && (gtk_toggle_button_get_active(g->btn_line_tool) || gtk_toggle_button_get_active(g->btn_curve_tool)))
  {
    // always end dragging before manipulating the path list to avoid
    // dangling pointers
    end_drag(g);
    if(!g->temp)
    {
      ...
      else
      {
        ...
        g->temp = alloc_move_to(module, pt);
        if(!g->temp) goto done;
        g->temp->warp.radius = pt + (dt_conf_key_exists(CONF_RADIUS) ? dt_conf_get_float(CONF_RADIUS)
                                                                     : GET_UI_WIDTH(DEFAULT_RADIUS));
        g->temp->warp.strength = pt + GET_UI_WIDTH(DEFAULT_STRENGTH);
      }
    }
    g->last_hit = NOWHERE;
  ...
  dt_pthread_mutex_unlock(&g->lock);
  if(handled) sync_pipe(module, TRUE);
  return handled;
}
...
int button_released(struct dt_iop_module_t *module, double x, double y, int which, uint32_t state)
{
  ...
  if(which == 3)
  {
    ...
    {
      gtk_toggle_button_set_active(g->btn_node_tool, !gtk_toggle_button_get_active(g->btn_node_tool));
      handled = 1;
      goto done;
    }
  ...
  if(gtk_toggle_button_get_active(g->btn_node_tool))
  {
    ...
    if(which == 1 && (g->last_mouse_mods == (GDK_MOD1_MASK | GDK_CONTROL_MASK)) && !dragged)
    {
      if(g->last_hit.layer == DT_LIQUIFY_LAYER_PATH)
      {
        // change segment
  ...
  if(which == 1) g->last_button1_pressed_pos = -1;
  g->last_hit = NOWHERE;
  dt_pthread_mutex_unlock(&g->lock);
  if(handled)
  {
...
static void btn_make_radio_callback(GtkToggleButton *btn, dt_iop_module_t *module)
{
  ...
  if(gtk_toggle_button_get_active(btn))
  {
    ...
    if(btn == g->btn_point_tool) dt_control_hinter_message(darktable.control, _("click and drag to add point"));
    if(btn == g->btn_line_tool) dt_control_hinter_message(darktable.control, _("click to add line"));
    if(btn == g->btn_curve_tool) dt_control_hinter_message(darktable.control, _("click to add curve"));
    if(btn == g->btn_node_tool) dt_control_hinter_message(darktable.control, _("click to edit nodes"));
  }
...
void gui_init(dt_iop_module_t *module)
{
  ...
  g->btn_node_tool = GTK_TOGGLE_BUTTON(
      dtgtk_togglebutton_new(_liquify_cairo_paint_node_tool, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER));
  ...
  g->btn_curve_tool = GTK_TOGGLE_BUTTON(
      dtgtk_togglebutton_new(_liquify_cairo_paint_curve_tool, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER));
  ...
  g->btn_line_tool = GTK_TOGGLE_BUTTON(
      dtgtk_togglebutton_new(_liquify_cairo_paint_line_tool, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER));
  ...
  g->btn_point_tool = GTK_TOGGLE_BUTTON(
      dtgtk_togglebutton_new(_liquify_cairo_paint_point_tool, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER));
  ...
  dt_liquify_layers[DT_LIQUIFY_LAYER_CENTERPOINT].hint
      = _("click and drag to move - click: show/hide feathering controls\n"
