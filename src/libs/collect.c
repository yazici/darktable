...
static gboolean
match_string_exact (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
...
static gboolean
entry_autocompl_match_selected (GtkEntryCompletion *widget, GtkTreeModel *model, GtkTreeIter *iter, dt_lib_collect_t *d)
{
  gchar *text;
...
static gboolean
entry_autocompl_match (GtkEntryCompletion *completion, const gchar *key, GtkTreeIter *iter, gpointer data)
{
  gchar *text;
...
static void tags_view (dt_lib_collect_rule_t *dr)
{
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW)
  {
    if(strchr((const char *)sqlite3_column_text(stmt, 0),'|')==0)
    {
      ...
      GtkTreeIter iter2;
      gtk_list_store_append(GTK_LIST_STORE(d->listmodel), &iter2);
      gtk_list_store_set(GTK_LIST_STORE(d->listmodel), &iter2, DT_LIB_COLLECT_COL_TEXT, sqlite3_column_text(stmt, 0),
...
list_view (dt_lib_collect_rule_t *dr)
{
  ...
  switch(property)
  {
    ...
    case DT_COLLECTION_PROP_TITLE: // title
      snprintf(query, sizeof(query), "select distinct value, 1 from meta_data where key = %d order by value", DT_METADATA_XMP_DC_TITLE);
      break;
    case DT_COLLECTION_PROP_DESCRIPTION: // description
      snprintf(query, sizeof(query), "select distinct value, 1 from meta_data where key = %d order by value", DT_METADATA_XMP_DC_DESCRIPTION);
      break;
    case DT_COLLECTION_PROP_CREATOR: // creator
      snprintf(query, sizeof(query), "select distinct value, 1 from meta_data where key = %d order by value", DT_METADATA_XMP_DC_CREATOR);
      break;
    case DT_COLLECTION_PROP_PUBLISHER: // publisher
      snprintf(query, sizeof(query), "select distinct value, 1 from meta_data where key = %d order by value", DT_METADATA_XMP_DC_PUBLISHER);
      break;
    case DT_COLLECTION_PROP_RIGHTS: // rights
      snprintf(query, sizeof(query), "select distinct value, 1 from meta_data where key = %d order by value ", DT_METADATA_XMP_DC_RIGHTS);
      break;
...
static void
update_selection (dt_lib_collect_rule_t *dr, gboolean exact)
{
  dt_lib_collect_t *d = get_collect(dr);
...
static void
entry_changed (GtkEditable *editable, dt_lib_collect_rule_t *d)
{
