...
/*
static gboolean
match_string2 (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
  dt_lib_collect_rule_t *dr = (dt_lib_collect_rule_t *) data;
  gchar *str = NULL;
...
static gboolean
expand_select_row (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, GtkTreeView *view)
{
  gboolean state;
  gtk_tree_model_get (model, iter, DT_LIB_COLLECT_COL_VISIBLE, &state, -1);
...
static void
expand_tree (GtkTreeView *view, dt_lib_collect_rule_t *dr, gboolean select)
{
  GtkTreeModel *model = gtk_tree_view_get_model (view);
  
  gtk_tree_selection_unselect_all(gtk_tree_view_get_selection(view));
  gtk_tree_view_collapse_all(view);
  if (select) gtk_tree_model_foreach (model, (GtkTreeModelForeachFunc)expand_select_row, view);
  else gtk_tree_model_foreach (model, (GtkTreeModelForeachFunc)expand_row, view);
...
//   gtk_widget_set_size_request(self->widget, 100, -1);
  ...
  GtkTreeModel *treemodel = GTK_TREE_MODEL(gtk_tree_store_new(DT_LIB_COLLECT_NUM_COLS, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_INT, G_TYPE_BOOLEAN));
