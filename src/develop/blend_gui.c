...
void dt_iop_gui_init_blending(GtkWidget *iopw, dt_iop_module_t *module)
{
  ...
  if(module->flags() & IOP_FLAGS_SUPPORTS_BLENDING)
  {
    ...
    dt_bauhaus_widget_set_label(bd->masks_blur_modes_combo, _("blur mode"), _("blur mode"));

    dt_bauhaus_combobox_add(bd->masks_blur_modes_combo, _("Gaussian"));
    bd->masks_blur_modes = g_list_append(bd->masks_blur_modes, GUINT_TO_POINTER(DEVELOP_MASK_BLUR_GAUSSIAN));

    dt_bauhaus_combobox_add(bd->masks_blur_modes_combo, _("guided filter"));
    bd->masks_blur_modes = g_list_append(bd->masks_blur_modes, GUINT_TO_POINTER(DEVELOP_MASK_BLUR_GUIDED_FILTER));

    dt_bauhaus_combobox_set(bd->masks_blur_modes_combo, 0);
    gtk_widget_set_tooltip_text(bd->masks_blur_modes_combo, _("blur mask by chosen method"));
    ...
    bd->contrast_slider = dt_bauhaus_slider_new_with_range(module, 0.0, 1.0, 0.01, 0.0, 2);
    dt_bauhaus_widget_set_label(bd->contrast_slider, _("blend"), _("mask contrast enhancement"));
    dt_bauhaus_slider_set_format(bd->contrast_slider, "%.2f");
    gtk_widget_set_tooltip_text(bd->contrast_slider, _("contrast enhancement of blend mask"));
    g_signal_connect(G_OBJECT(bd->contrast_slider), "value-changed",
                     G_CALLBACK(_blendop_blendif_contrast_callback), bd);


    bd->showmask = dtgtk_togglebutton_new(dtgtk_cairo_paint_showmask, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
